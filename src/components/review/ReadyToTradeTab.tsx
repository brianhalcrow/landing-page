...
export const ReadyToTradeTab = () => {
  const [isMounted, setIsMounted] = useState(true);

  const { data: tradeRequests = [], refetch } = useQuery({
    queryKey: ['trade-requests-ready'],
    queryFn: async () => {
  ...
  useEffect(() => {
    console.log("🏁 ReadyToTradeTab mounted");
    refetch();

    return () => {
      console.log("🔚 ReadyToTradeTab unmounted");
    };
  }, [refetch]);
...
export default ReadyToTradeTab;
